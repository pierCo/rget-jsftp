...
var FTPPool = function (name, connectionData, maxConnections, idleTimeoutMillis, logger) {
    ...
    if (typeof connectionData === 'undefined') {
        throw new Error('No FTP (' + name + ')parameters defined.');
    }
    ...
    function createFtpConnection(callback) {
        ...
        try {
            ...
            LOG.info('FTP (' + name + ') > Connect to : %s:%d', connectionData.host, connectionData.port);
        } catch (e) {
            err = e;
        }
        if (err) {
            ...
            LOG.info('FTP (' + name + ') > Connect with username : %s', connectionData.username);
    ...
    function destroyFtpConnection(ftp) {
        if (typeof ftp !== 'undefined') {
            ftp.raw.quit(function (err) {
                ...
                LOG.info('FTP (' + name + ') > Disconnected');
            });
        }
    }
